...
- 任何开发工作开始前先读取最近 10 个 git commit 的**完整 message**（`git log -10`，禁止用 `--oneline`），了解近期迭代方向、已完成的工作和决策上下文
  - 如有必要，继续深入查看相关 commit 的 diff 和 PR 描述，避免与刚完成的工作冲突或重复
  - 如果涉及 git 维护，先查 git log 完整内容
...
Commit message 就是 changelog，必须认真对待，不能草率。格式：

```
type: 首行概要（简洁但完整）

解释 why 和 what，按文件/区域分点列出具体变更。
用 - 开头的 bullet，格式为「文件/区域: 描述」。
```

- **首行**：`type: summary`，type 为 feat/fix/chore/docs/test；可用 em-dash 补充子标题
- **正文**：空一行后写 body，解释动机和影响；按文件或功能区域用 `-` 分点，每点格式为 `file.tsx: 做了什么`
- **禁止**：只写一行 subject 没有 body（除非是真正的一行改动如 typo fix）
- 参考项目已有 commit 风格，保持一致
